# Structural Synthesis Dynamics: A New Mathematical and Physical Theory
...
### 1.2 The SSD Solution

SSD throws away the assumption that geometry and matter are separate. Instead, it starts with one fundamental entity: the **Structure-State pair** \((\rho, \psi)\).
...
Any new theory must reproduce the successes of existing physics. SSD preserves:

| Principle | Status in SSD |
|-----------|--------------|
...
**What this means:** In ordinary physics, we assume spacetime exists independently of matter. In SSD, spacetime *is* the structure field \(\rho\). Without \(\rho\), there is no spacetime. Without \(\psi\), there is no matter. They are two aspects of the same thing.
...
\[\mathcal{H}_\rho = \left\{ \psi : M \to \mathbb{C} \;\Big|\; \int_M \frac{\lvert\psi\rvert^2}{\rho}\,d^4x < \infty \right\}, \tag{1}\]
...
Since \(d\tau = dx/\rho\), we have \(d^4x/\rho = d^4\tau\), and equation (2) becomes \(\int d^4\tau \, \psi_1^* \psi_2\) — the standard inner product. This means SSD uses the natural geometry defined by \(\rho\) itself.
...
\[S[\rho, \psi] = \int_M d^4x \left[ \frac{i\hbar}{2\rho}\left(\psi^* \partial_t \psi - \psi \partial_t \psi^*\right) - \frac{\hbar^2}{2m} \frac{\nabla\psi^* \cdot \nabla\psi}{\rho^3} - \frac{V(\tau)}{\rho} \lvert\psi\rvert^2 - \frac{\lambda}{\rho} \lvert\psi\rvert^4 - M_{\rm S}^4 V_{\rm struct}(\rho) + \frac{1}{2\kappa} (\partial_\mu\rho)(\partial^\mu\rho) - \frac{1}{2}\Lambda_{\rm bare} M_{\rm S}^2 \rho^4 \right]. \tag{3}\]
...
| \(-\frac{V(\tau)}{\rho}\lvert\psi\rvert^2\) | External potential energy |
| \(-\frac{\lambda}{\rho}\lvert\psi\rvert^4\) | Coupling between quantum matter and structure |
...
2. Varying with respect to \(\psi^*\):
   ...
   - The coupling term gives \(+4\lambda\lvert\psi\rvert^2\psi/\rho^5\).
3. Setting the variation to zero:
   \[i\hbar\partial_t\psi - \frac{\hbar^2}{m\rho^3}\nabla^2\psi - \frac{2V}{\rho^3}\psi - \frac{4\lambda}{\rho^5}\lvert\psi\rvert^2\psi = 0\]
...
\[\Box \rho = \kappa \left( \frac{\lambda}{2} \lvert\psi\rvert^4 + V'(\rho) - \Lambda_{\rm bare} M_{\rm S}^2 \rho^3 \right). \tag{7}\]
...
\[\begin{cases} i\hbar \partial_t \psi = -\frac{\hbar^2}{m} \nabla^2 \psi + V(\tau)\psi + \lambda \lvert\psi\rvert^2 \psi & \text{(Structure-Schrödinger)} \\ \Box\rho = \kappa \left( \frac{\lambda}{2} \lvert\psi\rvert^4 + V'(\rho) - \Lambda_{\rm bare} M_{\rm S}^2 \rho^3 \right) & \text{(Structure-field)} \end{cases} \tag{8}\]

**What this means:** The quantum state \(\psi\) evolves according to a Schrödinger equation where the Laplacian is weighted by \(\rho\). The structure field \(\rho\) evolves according to a wave equation (the Klein-Gordon equation) where the source is the quantum energy density \(\lvert\psi\rvert^4\) plus the structural potential. They are coupled: \(\psi\) creates \(\rho\), and \(\rho\) guides \(\psi\).
...
4. The coupling term \(-\frac{\lambda}{\rho}\lvert\psi\rvert^4\) appears in both \(E_{\rm quantum}\) and \(E_{\rm structural}\) with opposite signs, so it cancels in the total.
...
**Physical meaning:** When quantum effects are small, SSD reproduces general relativity. This is the correspondence principle: the new theory matches the old theory in the appropriate limit.
...
1. In the weak-field limit (\(\rho = \rho_0 + \delta\rho\), \(|\delta\rho|/\rho_0 \ll 1\)), equation (7) linearizes to:
    \[-\nabla^2 \delta\rho + V''(\rho_0) \delta\rho = \kappa \left(\frac{\lambda}{2}\lvert\psi\rvert^4 + \frac{\hbar^2}{m\rho_0^2}\nabla\psi^* \cdot \nabla\psi\right). \tag{6a}\]
...
8. **Distributed source:** For a disk with surface density \(\Sigma(R)\), the potential in the midplane is:
    ...
    In SSD, \(G\) is replaced by \(\kappa c^2/\rho_0\).
...
**Conclusion:** The steady-state structure-field equation gives Keplerian rotation curves for a point source and baryonic Tully-Fisher-like behavior for a disk. The observed flat rotation curves at large radii require a time-dependent dynamical effect during structure-field relaxation, which is within the SSD framework and is the subject of ongoing numerical investigation.
...
- SSD coupling: the action contains \(-\frac{\lambda}{\rho^4} |\Psi|^4\) where \(|\Psi|^2\) is the total system-apparatus wavefunction density.
...
**Physical meaning:** Wavefunction collapse is not a fundamental postulate in SSD. It is a deterministic, dynamical process. The apparent randomness comes from ignorance of the precise initial structure-field configuration. The Born rule is a statistical law over an ensemble of identically prepared systems, each with a different (unknown) initial \(\rho\)-configuration.

---

## V. Testable Predictions

SSD makes five specific, falsifiable predictions. All five follow directly from the postulates and derived theorems.
...
5. **Galaxy-specific rotation curves (Theorem 6):** Different galaxies with different disk scale lengths \(R_d\) and baryonic masses \(M_b\) should have different rotation curve shapes. The SSD prediction is \(v^4(R) = (\kappa c^2/\rho_0) M_b(R)/(\pi R_d)\), testable with detailed kinematic surveys.
...
Structural Synthesis Dynamics is a new theory built on four postulates. From these postulates, it derives:
...
## APPENDIX B: THE FIVE ASSUMPTIONS SSD REJECTS

| # | Assumption | Status in Modern Physics | Status in SSD |
|---|-----------|------------------------|---------------|
