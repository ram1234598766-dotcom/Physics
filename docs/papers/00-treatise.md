...
| Check | Result |
|---|---|
| \(\max\vertL_\rho\varphi_m - (-\mu_m)\varphi_m\vert\), \(m=1,\dots,4\) | \(3.6\times10^{-5}\) … \(6.9\times10^{-3}\) (grid) |
...
| \(m\) | \(\mu_m = (m\pi/\Lambda)^2\) | \(\omega_m\) | \(\max\vertL_\rho\varphi_m + \mu_m\varphi_m\vert\) (grid) |
|---|---|---|---|
...
| Check | Result |
|---|---|
| Skew connection \(\max\vertC + C^\top\vert\) | \(4.2\times10^{-6}\) |
...
| Migration suppression \(\vertC\vert/\text{bound}\) | \(\le 1\) everywhere |
...
| Migration suppression | \(\vertC\vert/\text{bound} = 0\) | Part III, §21.3 |
...
| Eigenvalue relation | \(\\vertL_\rho\varphi_m+\mu_m\varphi_m\\vert\) | 0 | \(3.6\times10^{-5}\)–\(6.9\times10^{-3}\) | grid-dependent |
...
| Evolution error (5 periods) | \(\\vertu(T)-u_{\mathrm{closed}}\\vert\) | \(2.4\times10^{-4}\) | Paper 02, §15 |
| Energy drift (100 periods) | \(\vertE(T)-E(0)\vert/E(0)\) | \(3.8\times10^{-14}\) | Paper 02, §15 |
| Eigenvalue perturbation (10%) | \(\delta\mu_m/\mu_m\) | \(0.05\%\) (corrected sign) | Paper 02, §15.2 |
| Eigenfunction residual | \(\\vert\varphi_m^{\mathrm{pert}}-\varphi_m^{\mathrm{exact}}\\vert\) | \(6\times10^{-5}\) | Paper 02, §15.2 |

**Table 37.3: Part III (causal network) verification**

| Check | Formula | Numerical result | Reference |
|---|---|---|---|
| Skew connection | \(\max\vertC+C^\top\vert\) | \(4.2\times10^{-6}\) | Paper 03, §21 |
...
| Migration suppression | \(\vertC_{jk}\vert/\text{bound}\) | \(\le1\) everywhere | Paper 03, §21.3 |
...
|---|---|---|---|
| Graded media | Transmission \(\vertT\vert\) | \(0.99997\) (vs \(1.000\) exact) | Paper 05, §XIII |
...
| 3D mode residuals | \(\\vertL_\rho\varphi_{m,n,p}-\mu\varphi\\vert\) | \(<10^{-3}\) | Paper 09, §IX |

**Table 37.7: Part VII (signal processing) verification**

| Check | Formula | Numerical result | Reference |
|---|---|---|---|
| Causal Parseval | \(\sum_j\vert\hat u_j\vert^2=\\vertu\\vert^2\) | \(<10^{-12}\) | Paper 10, §II |
...
|---|---|---|
| Exploration A (perturbation landscapes) | Paper 02, §XIII | \(\delta\mu_m\) vs \(\\vert\delta\rho\\vert\) for exponential/linear structures; confirms the corrected sign to .05\%\( \vert\)
| Exploration B (mode localization) | Paper 02, §XIV | \(\varphi_m(x)\) for \(m=1,\dots,8\) on \(\rho=e^x\), showing compression toward \(m=1\); nodal interval lengths in \(\tau\) vs \(\tau\) |
| Exploration C (energy migration) | Paper 03, §XXI | Time series of \(E(t)\) and \(E_j(t)=E_j/E\) for IEEE 14-bus under line stress; \(C(t)\) matrix heatmap; confirms migration suppression |
...
| Exploration E (Weyl law) | Paper 09, §XXI | \((\mu)/\mu^{d/2}\) vs \(\mu\) for \(d=2,3\); two-term correction residual; oscillatory corner-correction amplitude |
...
| Object | Paper | Treatise section |
|---|---|---|
| \rho f = \rho f'\( \vert 01 \vert Part I, �2 \vert\)
| \(\int f\,d\rho = \int f/\rho\,dx\) | 01 | Part I, �2 |
...
| \(u_{tt} = L_\rho u\) | 02 | Part II, §10 |
| Energy conservation \(E(t)\) | 02 | Part II, §11 |
| Green's function \( \vert 02 \vert Part II, �12 \vert\)
| Eigenvalue perturbation | 02 | Part II, �13 |
| Mass conservation \(m(t)\) | 03 | Part III, §16 |
| Contraction bound | 03 | Part III, �16 |
| \(C_{jk} = \langle\varphi_j,\dot\varphi_k\rangle\) | 03 | Part III, §17 |
...
| Migration suppression | 03 | Part III, �18 |
| Eigenvalue flow \(\dot\lambda_j\) | 03 | Part III, �19 |
| [u,\rho]\( action \vert 04 \vert Part IV, �22 \vert\)
| Hamiltonian [u,\pi,\rho]\( \vert 04 \vert Part IV, �23 \vert\)
| Noether momentum \(P(t)\) | 04 | Part IV, §23 |
| Structure stationarity | 04 | Part IV, �22 |
| Coupled equation (4.6) | 04 | Part IV, �24 |
| Poisson bracket \(\{\cdot,\cdot\}\) | 04 | Part IV, �XII |
| Gauge theory | 04 | Part IV, �XIII |
| Matched grading | 05 | Part V, �26 |
| Energy flux \( \vert 05 \vert Part V, �26 \vert\)
| Transmission coefficient | 05 | Part V, �X |
| Design formula \(\Lambda=\pi c_0/\omega\) | 05 | Part V, �27 |
| Synchronization rate | 06 | Part V, �27 |
| Time-to-synchronization | 06 | Part V, �27 |
