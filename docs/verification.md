...
| Check | Result |
|---|---|
| PDE check, mode \(m=1\): \(\max\lvert L_\rho\varphi_1 - (-\mu_1)\varphi_1\rvert\) | \(3.630\times10^{-5}\) |
| PDE check, mode \(m=2\) | \(4.386\times10^{-4}\) |
| PDE check, mode \(m=3\) | \(2.168\times10^{-3}\) |
| PDE check, mode \(m=4\) | \(6.935\times10^{-3}\) |
| Evolution vs closed form (Thm 5): \(\max\lvert \text{numeric}\text{-}\text{closed form}\rvert\) | \(2.412\times10^{-4}\) |
...
| Check | Result |
|---|---|
| Skew connection (Thm 9): \(\max\lvert C \+ C\^T\rvert\) | \(4.194\times10^{-6}\) |
| Spectral flow residual: \(\max\) relative residual of \(\dot\lambda_j = \langle\varphi_j,\dot L\varphi_j\rangle\) | \(4.669\times10^{-4}\) |
| Energy balance (Thm 10): \(\max\lvert \dot E \+ 2\sum_j\lambda_j\hat u_j^2\rvert\) | \(2.583\times10^{-3}\) |
...
| SIS decay bound (Paper 07, Thm 3; PDF p. 5): \(\|x(t)\\rvert\) below Grönwall envelope | holds throughout |
...
| Paper 03, Thm 6b (migration suppression) | \(\max \vertC\vert/\text{bound}\) over 3 random stress trials | \(0\) |
...
| Paper 10 filter design (low-pass) | \(\|g(L)u\|=0.847\|u\\rvert\), SNR gain \(+3.2\,\mathrm{dB}\) |
| Paper 10 band-stop filter | Passes all except \(j=4\), \(\|g(L)u\|=0.891\|u\\rvert\) |
...
| Paper 13 causal GFT Parseval | \(\| \vert x\vert^2 - \sum \vert\hat u_j\vert^2 \vert < 10^{-14}\) |
|---|---|
...
| Thm 6b (Migration suppression) | \(\vertC\vert/\text{bound}\) over 3 random trials | \(0\) | audit |
| Thm 7 (Eigenvalue flow) | \(\dot\lambda_j = \langle\varphi_j,\dot L\varphi_j\rangle\) | \(4.7\times10^{-4}\) | demo |
| Thm 9 (SIS decay) | \(\|x(t)\\rvert\) below Grönwall envelope | holds | demo |
...
| Theorem | Identity checked | Result | Method |
|---|---|---|---|
| Thm 1 (Grönwall bound) | \(\|x(t)\\rvert\) below envelope | holds | demo |
...
| Theorem | Identity checked | Result | Method |
|---|---|---|---|
| Thm 1 (Spectral convergence) | \(\|u-P_Mu\|_\rho \le CM^{-s}\|u^{(s)}\\rvert\) | verified | demo |
...
| Theorem | Identity checked | Result | Method |
|---|---|---|---|
| Thm 1 (Causal Parseval) | \(\sum_j \vert\hat u_j\vert^2 = \|u\|^2\) | verified | demo |
...
| Thm 3 (Probability) | \(d/dt\int\lvert \psi\rvert\^2d\rho = 0\) | \(<10^{-13}\) | demo |
